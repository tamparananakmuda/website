...
const articleFiles = [
  {
    file: 'content/articles/kehidupan/pulang-bukan-gagal-terbang-sistem-hancurkan-landasan.md',
    categoryId: CATEGORY_KEHIDUPAN,
    subcategorySlug: 'kehidupan-keluarga',
  },
  {
    file: 'content/articles/karier/skill-not-school-tapi-sistem-tetap-pakai-ijazah.md',
...
async function insert() {
  for (const cfg of articleFiles) {
    ...
    // Use status from frontmatter, but if publishedAt is past, set to published
    const pubDate = new Date(f.publishedAt);
    const now = new Date();
    const status = pubDate <= now ? 'published' : f.status;
    ...
    console.log('INSERTED:', f.slug, '| status:', status, '| publishedAt:', f.publishedAt);
  }
