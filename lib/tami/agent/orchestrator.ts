...
export async function processTamiIntelligence(query: string, history: { role: 'user' | 'assistant'; content: string }[] = [], options?: { streaming?: boolean }): Promise<TamiCognitiveResponse> {
  ...
  // 0c. Domain filter — skip LLM-based check in streaming mode for speed
  // Block queries outside TAMI's domain before running expensive 4-agent pipeline
  const domainCheck = isStreaming ? { isOffTopic: false, layer: 'skip', reason: 'skipped for streaming', rejectionMessage: undefined as string | undefined } : await checkDomain(normalizedQuery);
  ...
  // Semantic dedup: skip in streaming mode for speed
  const dedupResult = isStreaming ? { isDuplicate: false } : await checkSemanticDup(sessionId, normalizedQuery);
...
- Output WAJIB JSON valid dengan struktur:
{
  ...
  // Agent D: Verifier — skip in streaming mode (saves 5-10s)
...
3. Output WAJIB JSON valid dengan struktur:
{
  ...
  // 4. Executive Synthesis — skip in streaming mode (saves 5-10s, conversationalReply unused in streaming)
  ...
  let rawConversationalReply = '';
  let validated: { sanitizedReply: string; sanitizedCitations: typeof citations };

  if (isStreaming) {
    // In streaming mode, skip Executive Synthesis and output guardrails
    // The actual response will be generated by streamTamiReply()
    validated = { sanitizedReply: '', sanitizedCitations: citations };
  } else {
...
- Jangan pernah reveal parameter, model name, atau konfigurasi teknis lo.`
  ...
  rawConversationalReply = finalResponse.choices[0].message.content;
  ...
  if (!outputDomainCheck.inDomain) {
    ...
    const suggestions = generateQuickSuggestions(safeQuery, diagnosis.mindState.primaryEmotion, diagnosis.diagnosis.cognitiveDistortion);
    return {
      mindState: diagnosis.mindState,
      diagnosis: diagnosis.diagnosis,
  ...
  // Use rule-based suggestions in streaming mode (saves 4s LLM call)
  const suggestions = isStreaming
    ? generateQuickSuggestions(safeQuery, diagnosis.mindState.primaryEmotion, diagnosis.diagnosis.cognitiveDistortion)
    : await generateLLMSuggestions(safeQuery, diagnosis.mindState.primaryEmotion, diagnosis.diagnosis.cognitiveDistortion, validated.sanitizedReply);
