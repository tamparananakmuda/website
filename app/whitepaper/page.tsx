      ...
      <section className="relative w-full overflow-hidden">
        ...
        <div className="relative z-10 mx-auto max-w-6xl px-4 py-20 md:py-28 lg:py-36">
          <div className="mb-8 inline-flex items-center gap-2.5 rounded-full border border-amber-500/25 bg-amber-500/[0.07] px-4 py-1.5">
            ...
            <span className="text-[11px] font-semibold uppercase tracking-[0.25em] text-amber-600 dark:text-amber-400/90">Riset &amp; Analisis</span>
          </div>
          ...
          {allReports.length > 0 && (
            <div className="mt-12 flex flex-wrap items-center gap-x-10 gap-y-4">
              <div className="flex items-center gap-3">
                <FileText size={18} className="text-amber-600/70 dark:text-amber-500/60" />
                <div>
                  <div className="font-display text-2xl font-bold text-foreground dark:text-white">{allReports.length}</div>
                  <div className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground/70 dark:text-white/30">Laporan</div>
                </div>
              </div>
              <div className="h-8 w-px bg-border dark:bg-white/10" />
              <div className="flex items-center gap-3">
                <Clock size={18} className="text-amber-600/70 dark:text-amber-500/60" />
                <div>
                  <div className="font-display text-2xl font-bold text-foreground dark:text-white">{totalReadingTime}m</div>
                  <div className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground/70 dark:text-white/30">Total Baca</div>
                </div>
              </div>
              <div className="h-8 w-px bg-border dark:bg-white/10" />
              <div className="flex items-center gap-3">
                <Database size={18} className="text-amber-600/70 dark:text-amber-500/60" />
                <div>
                  <div className="font-display text-2xl font-bold text-foreground dark:text-white">{totalDataSources}+</div>
                  <div className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground/70 dark:text-white/30">Sumber Data</div>
                </div>
              </div>
            </div>
          )}
      ...
      {featuredReport && (
        <section className="mx-auto max-w-6xl px-4 pb-8 md:pb-12">
          <Link
            ...
            className="group relative block overflow-hidden rounded-2xl border border-border dark:border-white/[0.08] bg-gradient-to-br from-muted/50 dark:from-white/[0.03] to-transparent p-8 transition-all hover:border-amber-500/25 md:p-12"
          >
            ...
            <div className="grid gap-8 md:grid-cols-[1fr,auto] md:items-start">
              <div>
                <div className="mb-5 flex items-center gap-3">
                  ...
                  <span className="text-[11px] font-semibold uppercase tracking-[0.2em] text-amber-600/70 dark:text-amber-500/60">Latest</span>
                </div>

                <h2 className="mb-4 max-w-2xl font-display text-2xl font-bold leading-[1.15] text-foreground dark:text-white md:text-3xl lg:text-4xl lg:leading-[1.1]">
                  {featuredReport.title}
                </h2>

                {featuredReport.subtitle && (
                  <p className="mb-5 max-w-2xl text-base leading-relaxed text-muted-foreground dark:text-white/50 md:text-lg">
                    {featuredReport.subtitle}
                  </p>
                )}
                ...
                {featuredReport.keyFindings && featuredReport.keyFindings.length > 0 && (
                  <div className="mb-6 space-y-2">
                    {featuredReport.keyFindings.slice(0, 3).map((finding, i) => (
                      <div key={i} className="flex items-start gap-3 text-sm text-muted-foreground dark:text-white/40">
      ...
      {restReports.length > 0 && (
        <section className="mx-auto max-w-6xl px-4 py-12 md:py-20">
          <div className="mb-8 flex items-end justify-between">
            <div>
              <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.2em] text-amber-600/70 dark:text-amber-500/60">Semua Laporan</div>
              <h2 className="font-display text-2xl font-bold text-foreground dark:text-white md:text-3xl">
                Laporan Lainnya
              </h2>
            </div>
            <span className="font-mono text-sm text-muted-foreground/70 dark:text-white/30">{allReports.length} total</span>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {restReports.map((wp) => (
              <Link
                ...
                className="group relative flex flex-col overflow-hidden rounded-xl border border-border dark:border-white/[0.06] bg-card dark:bg-white/[0.015] p-6 transition-all hover:border-amber-500/20 hover:bg-muted/50 dark:hover:bg-white/[0.03]"
              >
                ...
                <h3 className="mb-3 font-display text-base font-bold leading-snug text-foreground dark:text-white transition-colors group-hover:text-amber-600 dark:group-hover:text-amber-400 md:text-lg">
                  {wp.title}
                </h3>

                {wp.subtitle && (
                  <p className="mb-4 line-clamp-2 text-sm leading-relaxed text-muted-foreground dark:text-white/40">
                    {wp.subtitle}
                  </p>
                )}
                ...
                <div className="mt-auto flex items-center gap-3 pt-4 text-[11px] text-muted-foreground/60 dark:text-white/25">
                  {wp.readingTime && (
      ...
      {allReports.length === 0 && (
        <section className="mx-auto max-w-6xl px-4 py-24">
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="font-display text-lg text-muted-foreground dark:text-white/40">TAM Report 2026 akan segera hadir.</p>
          </div>
        </section>
      )}
    </main>
